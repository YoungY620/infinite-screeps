...
### ❗ 核心原则：减轻后续 Session 的理解负担

**不要拘泥于现有文件结构！** 如果现有结构不合理，大胆重构、合并、删除整个文件。

**每次 Session 结束前，必须审视并重构知识库：**

1. **总结抽象** - 将具体事件抽象为通用经验
   - ✗ "在 tick 12345 发现 harvester 死了"
   - ✓ "当 Creep 数量 < 3 时，优先补充 harvester"

2. **合并重复** - 不要有多个文件说同一件事
   - 删除冗余内容
   - 合并相似的经验教训

3. **删除过时** - 过时/错误信息是干扰，必须立即删除
   - 已解决的问题不需要保留详细记录
   - 已废弃的策略直接删除
   - **发现错误内容立即修正或删除**

4. **结构清晰** - 让新 Session 能快速理解
   - 每个文件有明确职责
   - 重要信息放在文件开头

5. **及时更新** - 每次改动后立即同步更新所有相关文档
   - 代码改了 → 更新 code_design.md
   - 策略变了 → 更新 strategies.md
   - **不要留下“待更新”的文档，当场就改**
