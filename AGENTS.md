...
| 审视对象 | 关键问题 |
|----------|----------|
| **代码** | 这段逻辑真的正确吗？CPU 效率如何？有没有边界情况没处理？ |
| **设计** | 这个架构合理吗？是否过度复杂？有没有更简单的方案？ |
| **知识库** | 这些信息还准确吗？是否已过时？结论是否有依据？ |
| **策略** | 这个策略在当前情况下还适用吗？有没有更好的方案？ |

**发现问题时：**
- 不要因为"之前就是这样"而保留错误
- 大胆重构、简化、删除不合理的内容
- 记录为什么要改，供后续 Session 参考

### 先理解，再行动
**在修改任何代码之前，必须：**
1. **查阅文档** - 确认对相关机制的理解正确
2. **了解游戏内状态** - 通过 API 获取当前状态
3. **批判性分析现有代码** - 不要假设它是正确的
4. **评估 CPU 影响** - 预估改动对 CPU 消耗的影响
5. **本地验证** - 在私服测试后再上传

**禁止盲目修改代码，也禁止盲目信任现有代码！**

### 异常情况处理 【极其重要】

**发现异常或重大事件时，先确认是否是查询错误，再采取行动！**

```
发现异常 → 多个 API 交叉验证 → 确认真实情况 → 再采取行动
          ↑
    不要跳过这一步！
```
...
| 现象 | 可能的误判 | 正确做法 |
|------|------------|----------|
| `/api/user/rooms` 返回空 | "殖民地丢失了!" | 用 `/api/user/overview` 交叉验证 |
| `/api/game/room-objects` 失败 | "房间被摧毁了!" | 检查网络/参数，重试请求 |
| Creep 数量为 0 | "系统崩溃!" | 确认 API 返回格式，检查过滤条件 |
| 控制器等级为 0 | "被攻陷了!" | 检查是否查询了错误的房间 |

**验证步骤:**

1. **不要急于采取行动** - 重生、修改代码等操作不可逆
2. **多个 API 交叉验证** - 不同端点可能返回不同结果
3. **检查 API 调用参数** - shard、房间名、token 是否正确
4. **重试请求** - 网络问题可能导致临时失败
5. **查看原始返回** - 解析错误可能导致误判

**示例: 正确的状态检查流程**
```bash
# 1. 主要 API: 获取概览 (包含房间列表和统计)
curl "/api/user/overview?statName=energyHarvested&interval=8"
# → 检查 shards.shard3.rooms 是否有房间
...
# 3. 确认 API: 直接查询房间对象
curl "/api/game/room-objects?room=E13S35&shard=shard3"
# → 检查是否有自己的 Spawn 和 Controller
```
